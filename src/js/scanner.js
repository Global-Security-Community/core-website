  ...
  function doCheckIn(ticketCode) {
    ...
    .then(function(data) {
      ...
      if (data.status === 'checked_in') {
        ...
        resultEl.innerHTML = '<span class="icon" aria-hidden="true">' + GSCIcons.checkCircle + '</span> <strong>' + GSC.esc(data.attendeeName) + '</strong> checked in!';
        totalCheckedIn++;
        ...
        resultEl.innerHTML = '<span class="icon" aria-hidden="true">' + GSCIcons.alertTriangle + '</span> <strong>' + GSC.esc(data.attendeeName) + '</strong> already checked in at ' + GSC.esc(data.checkedInAt);
      } else {
        resultEl.style.backgroundColor = '#f8d7da'; resultEl.style.color = '#721c24';
        resultEl.innerHTML = '<span class="icon" aria-hidden="true">' + GSCIcons.xCircle + '</span> Invalid ticket code';
      }
