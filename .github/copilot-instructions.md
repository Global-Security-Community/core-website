...
- Use `sanitiseRichText()` from `../helpers/sanitise` for rich text HTML fields (e.g. event description)
- Use helpers from `../helpers/tableStorage` for all storage operations
- Use `logAudit()` from `../helpers/auditLog` for admin actions (fire-and-forget)
...
- **Quill.js 2.0** — Rich text editor, dashboard only (`cdn.jsdelivr.net`)
- **Leaflet.js 1.9.4** — Interactive maps, chapters listing page (`cdn.jsdelivr.net`)
- **QR code libraries** — Scanner page
- All loaded from CDN — no npm frontend dependencies
...
- Tables: `Chapters`, `Events`, `EventRegistrations`, `EventDemographics`, `EventBadges`, `ContactSubmissions`, `ChapterApplications`, `SessionizeCache`, `ChapterSubscriptions`, `CommunityPartners`, `AuditLog`
...
The chapters listing page (`/chapters/`) features an interactive map powered by **Leaflet.js** + **OpenStreetMap** tiles:

- **Map JS:** `src/js/chapter-map.js` — initializes map, creates markers, handles card ↔ marker interaction
- **Coordinates:** Each chapter's `index.md` has `latitude` and `longitude` frontmatter fields
- **Data attributes:** Chapter cards on the listing page have `data-lat`, `data-lng`, and `data-city` attributes read by the map JS
- **Leaflet CDN:** `cdn.jsdelivr.net` (already in CSP for Quill.js)
- **Map tiles:** `https://tile.openstreetmap.org` (added to CSP `img-src`)
- **Interaction:** Marker click → highlights/scrolls to chapter card; card hover → highlights marker + shows popup
- **Scroll zoom:** Disabled by default (prevents accidental zoom while scrolling page), enabled on map focus

**Auto-geocoding for new chapters:**
- The `generate-chapter.yml` workflow geocodes the chapter city via **Nominatim API** (OpenStreetMap, free, no API key)
- If geocoding fails, coordinates default to `0, 0` with a workflow warning annotation
- When editing a chapter via `updateChapter`, existing coordinates are read from the current file and preserved
...
- **CDN CSP gotcha:** `cdn.quilljs.com` redirects to `cdn.jsdelivr.net` — always verify the _final_ URL and add that to CSP
- **Map tiles CSP:** `https://tile.openstreetmap.org` must be in `img-src` — Leaflet loads tiles as `<img>` elements
...
- **Plain text fields** (title, location, etc.): use `sanitiseFields()` / `stripHtml()` — strips ALL HTML tags
- **Rich text fields** (event description): use `sanitiseRichText()` — allowlist of safe tags via `sanitize-html` npm package
  - Allowed tags: `p`, `br`, `strong`, `em`, `u`, `ol`, `ul`, `li`, `a`, `blockquote`, `span`
  - Links auto-tagged with `rel="noopener noreferrer" target="_blank"`
  - Classes restricted to Quill alignment/indent: `ql-align-center`, `ql-align-right`, `ql-align-justify`, `ql-indent-1/2/3`
- **URLs**: use `sanitiseUrl()` — validates http/https schemes only

---

## Rich Text Editor (Quill.js)

Event descriptions use the **Quill.js** rich text editor in the admin dashboard:

- **CDN:** `https://cdn.jsdelivr.net/npm/quill@2.0.3/dist/quill.js` (+ `quill.snow.css`)
- **Loaded on:** Dashboard page only (`src/dashboard/index.md`)
- **Toolbar:** Bold, Italic, Underline, Ordered/Unordered Lists, Alignment, Link, Clean
- **Initialization:** Wrapped in try/catch — if Quill fails to load, dashboard degrades gracefully
- **Reading content:** Use `quill.getText().trim()` for validation (empty check, length), `quill.root.innerHTML` for the HTML payload
- **Populating editor:** Set `quill.root.innerHTML = htmlContent` for edit forms
- **Storage:** HTML string stored in Azure Table Storage `description` field
- **Event page rendering:** `event-page.js` fetches description from `getEvent` API and sets `innerHTML` on `#event-description` div (progressive enhancement over SSR plain text in frontmatter)
- **YAML frontmatter:** Receives plain text version (`stripHtml(description)`) to avoid HTML escaping issues in YAML

---

## Audit Logging

All admin actions are logged to the `AuditLog` table in Azure Table Storage:

- **Helper:** `api/src/helpers/auditLog.js` — exports `logAudit()` and `getAuditLog()`
- **Pattern:** Fire-and-forget (never blocks or fails the parent operation)
- **Schema:** PartitionKey=`{entityType}_{entityId}`, RowKey=reverse timestamp + random suffix (newest first)
- **Logged fields:** `action`, `adminEmail`, `details` (JSON string), `timestamp`
- **Admin endpoint:** `GET /api/getAuditLog?eventId=X` or `?chapterSlug=X` (admin-only, route in `staticwebapp.config.json`)
- **Dashboard UI:** Activity Log section in event detail view

### Adding audit logging to a new admin function:

```js
const { logAudit } = require('../helpers/auditLog');

// After the action succeeds:
logAudit('event', eventId, 'event_updated', adminEmail, { title, date });
```

All 12 admin functions are instrumented: createEvent, deleteEvent, updateEvent, chapterApproval, updateChapter, adminRegister, updateRegistrationRole, checkIn, communityPartner, resendTicketEmail, issueBadges, eventAttendance.

---

## Event Model

Events are **single-date** (no start/end date range):

- **Dashboard forms:** Single "Date" field
- **Storage:** `date` field in Events table (the `endDate` field exists in storage for backward compatibility but is not sent from forms or displayed)
- **Display:** Event pages, event listings, and chapter pages all show a single date
- **Workflow:** `generate-event.yml` writes only `date` to event frontmatter
- **Grouping:** Events are grouped as Upcoming/Past by comparing event date to current date (not by status). Both SSR (Nunjucks template with `dateToMs`/`nowMs` filters) and client JS use date-based grouping for consistency
...
12. **CDN redirects break CSP** — Some CDNs redirect to different domains (e.g. `cdn.quilljs.com` → `cdn.jsdelivr.net`). Always verify the _final_ URL with `curl -sI <url>` and add that domain to CSP, not the redirect source
13. **Array.map callback gotcha** — `array.map(fn)` passes `(element, index, array)` to `fn`. If `fn` uses a second parameter (e.g. `renderCard(event, isPast)`), the index becomes that parameter. Wrap in `function(e) { return fn(e); }` to avoid
